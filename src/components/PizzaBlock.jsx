...
function PizzaBlock({ imageUrl, name, types, price, sizes, id }) {
    ...
    let [activeTypeHook, setActiveTypeHook] = React.useState(types[0]);
    let [activeSizeHook, setActiveSizeHook] = React.useState(0);
    ...
    return (
        ...
        <div
            ...
            <div className="pizza-block__selector">
                <ul>
                    {types.map((type, i) => {
                        let typeText = type === 0 ? 'тонкое' : 'традиционное';
                        let typeClassName = activeTypeHook === i ? 'active' : '';
                        return (
                            <li
                                key={i + (Math.random() * 10)}
                                onClick={() => onSelectType(i)}
                                className={typeClassName} > {typeText}</li>
                        )
                    })}
                </ul>
                <ul>
                    {sizes.map((size, i) => {
                        let sizeClassName = activeSizeHook === i ? ' active' : '';
                        return (
                            <li
                                key={i + (Math.random() * 10)}
